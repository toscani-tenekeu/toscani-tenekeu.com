...
const article = ref<CmsContent | null>(null)
...
function fallbackArticle(): Article | null {
  return fallbackArticles.find((item) => slugify(item.slug || item.title) === slug.value) || null
}

function fallbackMarkdown(item: Article) {
  return `# ${item.title}
...
This article is available in the editorial library and can be extended from the admin panel.`
}
...
async function load() {
  ...
  try {
    ...
    article.value = data
    htmlContent.value = data.htmlContent || await marked.parse(data.content || '')
    ...
    htmlContent.value = await marked.parse(fallbackMarkdown(fallback))
    comments.value = []
    error.value = ''
    setMeta(fallback.title, fallback.excerpt)
  } finally {
    loading.value = false
  }
}
  ...
  <div class="py-14">
    <div class="max-w-[920px] mx-auto px-6">
      ...
      <template v-else-if="article">
        <RouterLink to="/articles" class="inline-flex items-center gap-2 text-sm text-dim no-underline hover:text-link mb-8">
        ...
        <article class="bg-card border border-border rounded-lg overflow-hidden">
          <div v-if="article.coverImage" class="aspect-[16/8] bg-tint border-b border-border">
            <img :src="article.coverImage" :alt="article.title" class="w-full h-full object-cover" />
          </div>

          <div class="p-6 sm:p-10">
            <div class="flex flex-wrap gap-2 mb-5">
              <span v-for="tag in article.tags" :key="tag" class="inline-flex items-center font-mono text-[11px] uppercase tracking-wider text-dim px-2.5 py-1 border border-border rounded">
                {{ tag }}
              </span>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-x-3 gap-y-2 text-sm text-dim mb-8">
              <span class="inline-flex items-center gap-1.5"><FontAwesomeIcon :icon="['fas', 'user']" /> {{ article.author }}</span>
              <span class="text-border-hi">·</span>
              <span class="inline-flex items-center gap-1.5"><FontAwesomeIcon :icon="['fas', 'calendar']" /> {{ formatDateTime(article.createdAt) }}</span>
              <span class="text-border-hi">·</span>
              <span class="inline-flex items-center gap-1.5"><FontAwesomeIcon :icon="['fas', 'clock']" /> {{ article.readTime || '5 min read' }}</span>
            </div>

            <div class="article-markdown prose max-w-none prose-headings:font-light prose-headings:text-fg prose-p:text-fg2 prose-li:text-fg2 prose-a:text-green prose-strong:text-fg prose-code:text-fg prose-pre:bg-deep prose-pre:border prose-pre:border-border prose-pre:rounded-lg prose-pre:p-5">
        ...
        <section class="mt-8 bg-card border border-border rounded-lg p-6 sm:p-8">
          <div class="flex items-center justify-between gap-4 flex-wrap mb-5">
            ...
            <span class="text-xs uppercase tracking-[0.18em] text-dim">Month: {{ formatMonthYear(article.createdAt) }} · Time: {{ formatTime(article.createdAt) }}</span>
          </div>
          ...
          <form class="grid gap-3" @submit.prevent="submitComment">
            <div class="grid sm:grid-cols-2 gap-3">
              <input v-model="commentName" type="text" placeholder="Your name" class="bg-bg border border-border rounded-lg px-4 py-3 text-sm text-fg outline-none focus:border-green/50" required />
              <input v-model="commentEmail" type="email" placeholder="Email" class="bg-bg border border-border rounded-lg px-4 py-3 text-sm text-fg outline-none focus:border-green/50" required />
            </div>
            <textarea v-model="commentBody" rows="5" placeholder="Write your comment..." class="bg-bg border border-border rounded-lg px-4 py-3 text-sm text-fg outline-none focus:border-green/50 resize-y" required />
            <div class="flex items-center justify-between gap-3 flex-wrap">
              <p class="text-xs text-dim">Comments are reviewed before being published.</p>
              <button type="submit" class="cursor-pointer inline-flex items-center gap-2 text-sm font-medium px-5 py-2.5 rounded-full border border-green bg-green text-white hover:opacity-90 transition-opacity no-underline" :disabled="submitting">
                {{ submitting ? 'Sending…' : 'Send comment' }}
              </button>
            </div>
            <p v-if="submitMessage" class="text-sm" :class="submitMessage.includes('sent') ? 'text-green' : 'text-error'">
              {{ submitMessage }}
            </p>
          </form>
        </section>
...
.article-markdown :deep(:not(pre) > code) {
  background: var(--tint);
  border: 1px solid var(--border);
